...
export function validateBars(raw, { asOf = Date.now(), maxBars = 20000 } = {}) {
...
function resolveOutcome(bars, anchorIndex, horizon, sessionPolicy, timeframeMs) {
  ...
  if (["us_exchange", "cn_exchange"].includes(sessionPolicy)) {
    outcomeIndex = tradingDayOutcomeIndex(bars, anchorIndex, horizon.value, timeframeMs, sessionPolicy);
  } else {
    for (let index = anchorIndex + 1; index < bars.length; index += 1) {
      if (bars[index].end >= target) {
        outcomeIndex = index;
        break;
      }
    }
...
export function analyze(input) {
  ...
  for (let index = lookback - 1; index < queryStart - 1; index += 1) {
    const outcome = resolveOutcome(bars, index, decisionHorizon, sessionPolicy, timeframeSpec.ms);
    if (!outcome || outcome.actualEnd >= bars[queryStart].t) {
      rejectedImmature += 1;
      continue;
    }
    if (!goodWindow(bars, index - lookback + 1, outcome.outcomeIndex, timeframeSpec.ms, sessionPolicy)) {
      rejectedGaps += 1;
      continue;
    }
    const candidateFeatures = features(bars.slice(index - lookback + 1, index + 1));
    if (candidateFeatures.flat) continue;
    ...
    if (comparison.distance > 0.85) continue;
    const neutralPct = candidateFeatures.atr * 0.2 * Math.sqrt(outcome.future.length) * 100;
    candidates.push({
      index,
...
export function alignShape(bars, shape, {
  ...
  for (const hit of hits) {
    if (selected.every((current) => Math.abs(hit.index - current.index) >= lookback)) selected.push(hit);
...
export function parseCSV(text, timeframeMs = HOUR) {
  if (typeof text !== "string" || text.length > 5_000_000) throw new Error("CSV 最大 5 MB");
